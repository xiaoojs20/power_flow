...
from case_lf_py import build_case_lfreq_py
...
def run_pf_python():
    ...
    # Assuming VM is defined elsewhere or imported, e.g., from pypower.idx_bus
    # For example: from pypower.idx_bus import VM
    # If not, this line will cause a NameError.
    # For the purpose of this edit, we assume VM is available in the scope.
    ...
    for mode_name, mode_val in modes.items():
        ...
        for name, Ps in scenarios.items():
            try:
                ppc = build_case_lfreq_py(Ps, control_mode=mode_val)
                res = runpf(ppc, ppopt)
        ...
        for scale in np.arange(0.1, 1.6, 0.1):
            ...
            try:
                ppc_s = build_case_lfreq_py(Ps_sweep, control_mode=mode_val)
    ...
    print("PYPOWER scenarios and sweeps completed for all modes.")
